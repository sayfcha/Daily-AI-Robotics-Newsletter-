#!/usr/bin/env python3
"""
Daily VC & Startup Newsletter (France/Europe focus)
Fetches latest news on VC, startups, fundraising, and the European tech ecosystem.
"""
...
FEEDS = {
    ...
    "🇫🇷 Startups France": [
        ...
        ("BPI France", "https://www.bpifrance.fr/nos-actualites/rss"),
        ("Station F Blog", "https://stationf.co/feed"),
    ],
    "🇪🇺 Startups Europe": [
        ("Sifted", "https://sifted.eu/feed"),
        ("Tech.eu", "https://tech.eu/feed/"),
        ("EU-Startups", "https://www.eu-startups.com/feed/"),
        ("TechCrunch Europe", "https://techcrunch.com/region/europe/feed/"),
...
def is_recent(entry, hours: int = 48) -> bool:
    """Check if an entry is from the last N hours."""
...
def build_newsletter() -> tuple[str, str]:
    ...
    subject = f"🚀 Daily VC & Startup Briefing — {today}"
    ...
    <head>
        <style>
            ...
            .top-stories {{ background: #fff; border-radius: 12px; padding: 20px; margin-bottom: 24px; border-left: 4px solid #f5576c; }}
            .top-stories h2 {{ margin-top: 0; font-size: 18px; color: #f5576c; }}
            ...
            .top-stories a:hover {{ color: #f5576c; }}
            ...
            .article h3 a:hover {{ color: #f5576c; }}
    ...
    <body>
        <div class="header">
            <h1>🚀 Daily VC & Startup Briefing</h1>
            <p>{today} — France, Europe & beyond</p>
        </div>
    """
    ...
    html += """
        <div class="footer">
            <p>Généré automatiquement par ton VC Newsletter Bot 🚀</p>
...
def send_email(subject: str, html_body: str):
    ...
    text_body = "Votre newsletter quotidienne VC & Startups est disponible. Consultez-la dans un client email supportant le HTML."
...
def main():
    print("🚀 Building your daily VC & Startup newsletter...\n")
    subject, html = build_newsletter()

    if not SENDER_PASSWORD:
        print("⚠ No SENDER_PASSWORD set. Saving newsletter to file instead.")
        filename = f"newsletter_vc_{datetime.now().strftime('%Y-%m-%d')}.html"
